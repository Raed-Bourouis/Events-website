...
let getOneComment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "this id does not exist please try again" });
  }
};
...
let addComment = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "error occured while creating new comment" });
  }
};
...
let updateComment = async (req, res) => {
  ...
  try {
    let comment = await commentSchema.findByIdAndUpdate(commentId, value, {
      new: true,
      old: false,
    });
    res
      .status(200)
      .json({ message: "updated successfully, client is now : " + comment });
  } catch (e) {
    return res
      .status(404)
      .json({ message: "this id does not exist please try again" });
